    ...
    if args.monitoring_mode == "node":
        if args.monitoring_type == "json":
            print("JSON output (node): {}".format(
                getNode(args.object_name, args.exclude_name, args.match_label)))
        if args.monitoring_type == "discovery":
            print("Zabbix discovery (node): {}".format(
                zabbix.send(zabbixDiscoveryNode(args.kubernetes_name, getNode(args.object_name, args.exclude_name, args.match_label)))))
        if args.monitoring_type == "item":
            print("Zabbix item (node): {}".format(
                zabbix.send(zabbixItemNode(args.kubernetes_name, getNode(args.object_name, args.exclude_name, args.match_label)))))

    # Daemonset
    if args.monitoring_mode == "daemonset":
        if args.monitoring_type == "json":
            print("JSON output (daemonset): {}".format(
                getDaemonset(args.object_name, args.exclude_name, args.exclude_namespace, args.match_label)))
        if args.monitoring_type == "discovery":
            print("Zabbix discovery (daemonset): {}".format(
                zabbix.send(zabbixDiscoveryDaemonset(args.kubernetes_name, getDaemonset(args.object_name, args.exclude_name, args.exclude_namespace, args.match_label)))))
        if args.monitoring_type == "item":
            print("Zabbix item (daemonset): {}".format(
                zabbix.send(zabbixItemDaemonset(args.kubernetes_name, getDaemonset(args.object_name, args.exclude_name, args.exclude_namespace, args.match_label)))))

    # Volumes
    if args.monitoring_mode == "volume":
        if args.monitoring_type == "json":
            print("JSON output (volume): {}".format(
                getVolume(args.object_name, args.exclude_name, args.exclude_namespace, args.match_label)))
        if args.monitoring_type == "discovery":
            print("Zabbix discovery (volume): {}".format(
                zabbix.send(zabbixDiscoveryVolume(args.kubernetes_name, getVolume(args.object_name, args.exclude_name, args.exclude_namespace, args.match_label)))))
        if args.monitoring_type == "item":
            print("Zabbix item (volume): {}".format(
                zabbix.send(zabbixItemVolume(args.kubernetes_name, getVolume(args.object_name, args.exclude_name, args.exclude_namespace, args.match_label)))))
    
    # Deployment
    if args.monitoring_mode == "deployment":
        if args.monitoring_type == "json":
            print("JSON output (deployment): {}".format(
                getDeployment(args.object_name, args.exclude_name, args.exclude_namespace, args.match_label)))
        if args.monitoring_type == "discovery":
            print("Zabbix discovery (deployment): {}".format(
                zabbix.send(zabbixDiscoveryDeployment(args.kubernetes_name, getDeployment(args.object_name, args.exclude_name, args.exclude_namespace, args.match_label)))))
        if args.monitoring_type == "item":
            print("Zabbix item (deployment): {}".format(
                zabbix.send(zabbixItemDeployment(args.kubernetes_name, getDeployment(args.object_name, args.exclude_name, args.exclude_namespace, args.match_label)))))

    # Statefulset
    if args.monitoring_mode == "statefulset":
        if args.monitoring_type == "json":
            print("JSON output (statefulset): {}".format(
                getStatefulset(args.object_name, args.exclude_name, args.exclude_namespace, args.match_label)))
        if args.monitoring_type == "discovery":
            print("Zabbix discovery (statefulset): {}".format(
                zabbix.send(zabbixDiscoveryStatefulset(args.kubernetes_name, getStatefulset(args.object_name, args.exclude_name, args.exclude_namespace, args.match_label)))))
        if args.monitoring_type == "item":
            print("Zabbix item (statefulset): {}".format(
                zabbix.send(zabbixItemStatefulset(args.kubernetes_name, getStatefulset(args.object_name, args.exclude_name, args.exclude_namespace, args.match_label)))))

    # Cronjob
    if args.monitoring_mode == "cronjob":
        if args.monitoring_type == "json":
            print("JSON output (cronjob): {}".format(
                getCronjob(args.object_name, args.exclude_name, args.exclude_namespace, args.match_label)))
        if args.monitoring_type == "discovery":
            print("Zabbix discovery (cronjob): {}".format(
                zabbix.send(zabbixDiscoveryCronjob(args.kubernetes_name, getCronjob(args.object_name, args.exclude_name, args.exclude_namespace, args.match_label)))))
        if args.monitoring_type == "item":
            print("Zabbix item (cronjob): {}".format(
                zabbix.send(zabbixItemCronjob(args.kubernetes_name, getCronjob(args.object_name, args.exclude_name, args.exclude_namespace, args.match_label)))))
