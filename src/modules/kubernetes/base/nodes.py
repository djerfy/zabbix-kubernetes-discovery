...
def zabbixDiscoveryNodes(config):
    ...
    for node in kubernetesGetNodes(config):
        output = {"{#KUBERNETES_BASE_NODES_NAME}": node['name']}
        discovery['data'].append(output)

    return [[config['kubernetes']['name'], "kubernetes.base.nodes.discovery", json.dumps(discovery)]]
...
def zabbixItemsNodes(config):
    ...
    for node in kubernetesGetNodes(config):
        items.append([config['kubernetes']['name'], f"kubernetes.base.nodes.healthz[{node['name']}]", node['status']])
        items.append([config['kubernetes']['name'], f"kubernetes.base.nodes.capacity.cpu[{node['name']}]", node['capacity']['cpu']])
        items.append([config['kubernetes']['name'], f"kubernetes.base.nodes.capacity.memory[{node['name']}]", node['capacity']['memory']])
        items.append([config['kubernetes']['name'], f"kubernetes.base.nodes.capacity.pods[{node['name']}]", node['capacity']['pods']])
        items.append([config['kubernetes']['name'], f"kubernetes.base.nodes.allocatable.cpu[{node['name']}]", node['allocatable']['cpu']])
        items.append([config['kubernetes']['name'], f"kubernetes.base.nodes.allocatable.memory[{node['name']}]", node['allocatable']['memory']])
        items.append([config['kubernetes']['name'], f"kubernetes.base.nodes.allocatable.pods[{node['name']}]", node['allocatable']['pods']])
        items.append([config['kubernetes']['name'], f"kubernetes.base.nodes.current.pods[{node['name']}]", node['current']['pods']])
        items.append([config['kubernetes']['name'], f"kubernetes.base.nodes.current.podsUsed[{node['name']}]", node['current']['pods_used']])
        items.append([config['kubernetes']['name'], f"kubernetes.base.nodes.current.podsFree[{node['name']}]", node['current']['pods_free']])
